...
export const getRestaurantCuisines = async (req, res) => {
    try {
...
export const getNewRestaurants = async (req, res) => {
    try {
...
export const getRestaurantItemsByCategory = async (req, res) => {
    try {
        const { id } = req.params;
...
export const getRestaurantCategoriesById = async (req, res) => {
    try {
        const { id } = req.params;
        const restaurant = await Restaurant.findById(id);
        if (!restaurant) {
...
export const getUserCarts = async (req, res) => {
    try {
        ...
        const userCartsRestaurant = userCarts.map((cart) => {
...
export const addCartItemNote = async (req, res) => {
    try {
        ...
        await CartItem.findByIdAndUpdate(cartItemId, { $set: { note } }, { new: true });
...
export const removeCartItemNote = async (req, res) => {
    try {
...
export const getCategoryItems = async (req, res) => {
    try {
        ...
        const items = await Item.find({ category: categoryId }).populate("restaurantId");
        return res.status(200).json(items);
    }
    catch (error) {
        return res
