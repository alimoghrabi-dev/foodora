...
import { verifyEmailVerificationToken, verifyRestaurantRegistrationTokenCreateRoute, verifyToken, } from "../utils/token.js";
import { acceptAndReadyOrder, addNewItem, applyDiscount, completeInfo, createNewCategory, createNewRestaurant, creatingVerificationLink, editMenuItem, editRestaurantHours, editRestaurantInfo, getItemById, getOrderById, getRestaurantCategories, getRestaurantMenuItems, getRestaurantOrders, loginRestaurant, publishStore, removeDiscount, renderImage, toggleRestaurant, verifyEmail, verifyRestaurant, } from "../controllers/restaurant.controllers.js";
import { upload } from "../utils/multer.js";
import Restaurant from "../models/restaurants.model.js";
import cron from "node-cron";
import dayjs from "dayjs";
const restaurantRoutes = Router();
restaurantRoutes.post("/register", verifyRestaurantRegistrationTokenCreateRoute, createNewRestaurant);
...
restaurantRoutes.post("/add-item", verifyToken, upload.single("image"), addNewItem);
...
restaurantRoutes.put("/edit-item/:id", verifyToken, upload.single("image"), editMenuItem);
restaurantRoutes.put("/complete-info", verifyToken, upload.single("image"), completeInfo);
...
restaurantRoutes.get("/verify-email", verifyEmailVerificationToken, verifyEmail);
...
restaurantRoutes.put("/edit-info", verifyToken, upload.single("image"), editRestaurantInfo);
...
const isRestaurantTimeout = (openingHours) => {
    if (!openingHours) {
