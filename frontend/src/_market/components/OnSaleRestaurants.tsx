...
import { getOnSaleRestaurants } from "../../lib/actions";

const OnSaleRestaurants: React.FC = () => {
  ...
  const { data: restaurants, isPending } = useQuery({
    queryKey: ["on-sale-restaurants"],
    queryFn: async () => {
      return await getOnSaleRestaurants();
    },
  });
  ...
  return (
    ...
    >
      <h2 className="text-2xl primary-text font-semibold flex gap-x-2">
        <FaFire size={27} className="text-primary" />
        Hot Offers
      </h2>
...
export default OnSaleRestaurants;
